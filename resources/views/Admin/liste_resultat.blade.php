            ...
            <div class="card my-2 bg-light "> 
                <div class="card-header" style="background-color:#d0497a;">
                    <div class="text-center">
                        <h3 class="text-white">La Listes Des Resultats</h3>
                    </div>
                </div>
                    ...
                    <table id="mytable" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <tr class="text-center ">
                                ...
                                <td data-label="Résultat">{{$item->resultat ==null ?'pas encore':$item->resultat}}</td>
                                <td data-label="Action" class="d-flex justify-content-center align-items-center ">
                                        <a href="{{ $item->resultat != null ? route('admin.ValiderResultat', $item->id_operation) : '#' }}" class="btn btn-sm btn-success mx-2" 
                                            onclick="{{ $item->resultat == null ? 'alert(\'modifier le résultat premierement\')' : '' }}">
                                            <i class="fas fa-check"></i>
                                        </a>
                                    <a href="{{route('admin.ModifierResultat',$item->id_operation)}}" class="btn btn-sm btn-warning mx-2">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form id="{{$item->id_operation}}"  action="{{route('admin.SupprimerResultat',$item->id_operation)}}" method="POST">
                                        @method('DELETE')
                                        @csrf
                                    </form>
